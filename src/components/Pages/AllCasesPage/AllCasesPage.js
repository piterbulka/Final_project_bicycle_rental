          ...
          <tbody>
            {cases.map((caseItem) => (
              <tr key={caseItem._id}>
                ...
                <td>
                <Button variant="danger" className='me-2 mb-2' onClick={() => handleDelete(caseItem._id)}>Удалить</Button>
                  <Link to={`/cases/${caseItem._id}`} className="btn btn-success ml-2 me-2 mb-2">Подробнее</Link>
                </td>
              </tr>
            ))}
          </tbody>
